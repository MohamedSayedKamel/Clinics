...
@Service("presenceOfAnemiaBeanConverter")
public class PresenceOfAnemiaBeanConverter implements Converter<VisitDetailsResponse, PresenceOfAnemiaBean> {

	@Override
	public PresenceOfAnemiaBean convert(VisitDetailsResponse source) throws ConversionException {
		if(source ==null)
		{
			throw new ConversionException("source must not be null!");
		}
		
		PresenceOfAnemiaBean target = new PresenceOfAnemiaBean();
		
		target.setTotalHemoglobin(source.getTotalHemoglobin());

		return target;
	}

	@Override
	public PresenceOfAnemiaBean convert(VisitDetailsResponse source, PresenceOfAnemiaBean target)
