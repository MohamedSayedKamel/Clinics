...
@Service("aceInhibitorBeanConverter")
public class AceInhibitorBeanConverter implements Converter<VisitDetailsResponse, AceInhibitorBean> {

	@Override
	public AceInhibitorBean convert(VisitDetailsResponse source) throws ConversionException {
		if(source ==null)
		{
			throw new ConversionException("source must not be null!");
		}
		
		AceInhibitorBean target = new AceInhibitorBean();
		target.setAceInhibitorType(source.getAceInhibitorType());
		target.setAceInhibitorReason(source.getAceInhibitorReason());
		target.setAceInhibitorDrugs(source.getAceInhibitorDrugs());
	...
	@Override
	public AceInhibitorBean convert(VisitDetailsResponse source, AceInhibitorBean target)
