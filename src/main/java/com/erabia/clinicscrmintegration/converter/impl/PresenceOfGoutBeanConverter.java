...
@Service("presenceOfGoutBeanConverter")
public class PresenceOfGoutBeanConverter implements Converter<VisitDetailsResponse, PresenceOfGoutBean> {

	@Override
	public PresenceOfGoutBean convert(VisitDetailsResponse source) throws ConversionException {
		if(source ==null)
		{
			throw new ConversionException("source must not be null!");
		}
		
		PresenceOfGoutBean target =  new PresenceOfGoutBean();
		target.setUricAcidMeasurement(source.getUricAcidMeasurement());

		return target;
	}

	@Override
	public PresenceOfGoutBean convert(VisitDetailsResponse source, PresenceOfGoutBean target)
