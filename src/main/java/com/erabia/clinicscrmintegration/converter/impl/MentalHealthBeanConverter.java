...
@Service("mentalHealthBeanConverter")
public class MentalHealthBeanConverter implements Converter<VisitDetailsResponse, MentalHealthBean> {

	@Override
	public MentalHealthBean convert(VisitDetailsResponse source) throws ConversionException {
		if(source ==null)
		{
			throw new ConversionException("source must not be null!");
		}
		
		MentalHealthBean target = new MentalHealthBean();
		target.setScreenForDepressionAnxiety(source.getScreenForDepressionAnxiety());

		return target;
	}

	@Override
	public MentalHealthBean convert(VisitDetailsResponse source, MentalHealthBean target)
