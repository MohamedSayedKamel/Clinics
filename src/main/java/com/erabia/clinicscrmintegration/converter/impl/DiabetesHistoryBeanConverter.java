...
@Service("diabetesHistoryBeanConverter")
public class DiabetesHistoryBeanConverter implements Converter<VisitDetailsResponse, DiabetesHistoryBean> {

	@Override
	public DiabetesHistoryBean convert(VisitDetailsResponse source) throws ConversionException {
		if(source ==null)
		{
			throw new ConversionException("source must not be null!");
		}
		
		DiabetesHistoryBean target=new DiabetesHistoryBean();

		target.setDiabetesType(source.getDiabetesType());
		target.setStrdiabetesDateDiagnosed(source.getStrDiabetesDateDiagnosed());
		try {
			target.setDiabetesDateDiagnosed(source.getDiabetesDateDiagnosedFormat());
		} catch (DateFormattingException e) {
		
		}
		target.setStrComorbidities(source.getStrComorbidities());

		return target;
	}

	@Override
	public DiabetesHistoryBean convert(VisitDetailsResponse source, DiabetesHistoryBean target)
