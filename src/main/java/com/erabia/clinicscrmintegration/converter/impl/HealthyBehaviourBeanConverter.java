...
@Service("healthyBehaviourBeanConverter")
public class HealthyBehaviourBeanConverter implements Converter<VisitDetailsResponse, HealthyBehaviourBean> {

	@Override
	public HealthyBehaviourBean convert(VisitDetailsResponse source) throws ConversionException {
		if(source ==null)
		{
			throw new ConversionException("source must not be null!");
		}
		
		HealthyBehaviourBean target=new HealthyBehaviourBean();

		target.setWeight(source.getWeight());
		target.setHeight(source.getHeight());
		target.setBmi(source.getBmi());
		target.setWaistCircumference(source.getWaistCircumference());
		target.setNutritionType(source.getNutritionType());
		target.setPhysicalActivityType(source.getPhysicalActivityType());
		target.setSmokingStatus(source.getSmokingStatus());

		return target;
	}

	@Override
	public HealthyBehaviourBean convert(VisitDetailsResponse source, HealthyBehaviourBean target)
