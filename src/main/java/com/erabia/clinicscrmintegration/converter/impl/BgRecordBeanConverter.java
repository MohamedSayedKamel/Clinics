...
@Service("bgRecordBeanConverter")
public class BgRecordBeanConverter implements Converter<VisitDetailsResponse, BgRecordBean> {

	@Override
	public BgRecordBean convert(VisitDetailsResponse source) throws ConversionException {
		if(source ==null)
		{
			throw new ConversionException("source must not be null!");
		}
		
		BgRecordBean target = new BgRecordBean();
		
		target.setFastingBg(source.getFastingBg());
		target.setPostPrandialBg(source.getPostPrandialBg());
		target.setHypoglycemicEpisodes(source.getHypoglycemicEpisodes());

		return target;
	}

	@Override
	public BgRecordBean convert(VisitDetailsResponse source, BgRecordBean target)
