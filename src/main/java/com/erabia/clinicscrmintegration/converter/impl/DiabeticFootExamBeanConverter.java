...
@Service("diabeticFootExamBeanConverter")
public class DiabeticFootExamBeanConverter implements Converter<VisitDetailsResponse, DiabeticFootExamBean> {

	@Override
	public DiabeticFootExamBean convert(VisitDetailsResponse source) throws ConversionException {
		if(source ==null)
		{
			throw new ConversionException("source must not be null!");
		}
		
		DiabeticFootExamBean target = new DiabeticFootExamBean();
		target.setDiabeticFootExamSensation(source.getDiabeticFootExamSensation());
		target.setDiabeticFootExamPulses(source.getDiabeticFootExamPulses());
		target.setDiabeticFootExamSkin(source.getDiabeticFootExamSkin());
		target.setDiabeticFootExamOther(source.getDiabeticFootExamOther());
		target.setStrNeuropathy(source.getStrNeuropathy());

		return target;
	}

	@Override
	public DiabeticFootExamBean convert(VisitDetailsResponse source, DiabeticFootExamBean target)
